...
export default {
  async fetch(request, env) {
    ...
    if (p === "/api/memory" && request.method === "POST") {
      const d = await request.json();
      const kid = await kvAdd("m", {content:d.content,tags:(d.tags||"").split(",").map(function(t){return t.trim();}).filter(function(t){return t;}),category:d.category||"",importance:d.importance||0,event_date:d.event_date||""});
    ...
    if (p === "/api/memory" && request.method === "DELETE") {
      await kvDel("m", url.searchParams.get("id"));
      return new Response(JSON.stringify({ok:1}),{headers:h});
    }
    if (p === "/api/memory/read") {
      const d = await kvGet("m", url.searchParams.get("id"));
    ...
    if (p === "/mcp" && request.method === "POST") {
      ...
      if (method === "initialize") return new Response(JSON.stringify({jsonrpc:"2.0",id:id,result:{protocolVersion:"2024-11-05",capabilities:{tools:{}},serverInfo:{name:"ember-home",version:"8.0.0"}}}),{headers:h});
      ...
      if (method === "tools/call") {
        ...
        if (tn === "send_nyx_command") {
          ...
          const kid=await kvAdd("m",{content:a.content,tags:(a.tags||"").split(",").map(function(t){return t.trim();}).filter(function(t){return t;}),category:a.category||"",importance:a.importance||0,event_date:a.event_date||""});
          r=JSON.stringify({ok:1,id:kid});
        }
        else if (tn === "memory_search") {
          const q=(a.query||"").toLowerCase();const lim=a.limit||20;var idx=[];const iv=await env.KV.get("memory:index");if(iv)idx=JSON.parse(iv);var scored=[];
          for(const kid of idx){const v=await env.KV.get("m:"+kid);if(!v)continue;const m=JSON.parse(v);var sc=0;if((m.content||"").toLowerCase().includes(q))sc+=10;if((m.tags||[]).join(" ").toLowerCase().includes(q))sc+=5;if((m.category||"").toLowerCase().includes(q))sc+=3;if(sc>0)scored.push({mem:m,score:sc});}
          scored.sort(function(x,y){return y.score-x.score;});var cnt=scored.length>lim?lim:scored.length;r=JSON.stringify({query:q,count:cnt,memories:scored.slice(0,lim).map(function(x){return x.mem;})});
        }
          ...
          const d=await kvUpdate("m",a.memory_id,u);r=d?JSON.stringify({ok:1}):JSON.stringify({error:"not found"});
        }
